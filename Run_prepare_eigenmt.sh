...
for i in ${chr}
do
	echo "tabix -h ${vcf} chr${i} | python scripts/vcf2matrixeqtl.py | gzip -c > ${dir}/genotypes/chr${i}.txt.gz" | qsub -cwd -V -l mem=5G -N eigen_gt_${i}
	echo "tabix -h ${vcf} chr${i} | python scripts/vcf2matrixeqtl_pos.py | gzip -c > ${dir}/positions/chr${i}.txt.gz" | qsub -cwd -V -l mem=5G -N eigen_pos_${i}
done

# 2) generate phenotype position files - C position in bed files = bed + 1
